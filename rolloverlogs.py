...
class ParallelTimedRotatingFileHandler(logging.handlers.TimedRotatingFileHandler):
    ...
    def __init__(self, filename, when='h', interval=1, backupCount=0, encoding=None, delay=False,
        ...
        super().__init__(filename=self.calculateFileName(currenttime), when=when, interval=interval, backupCount=backupCount,
...
def create_timed_rotating_log(path, logger_name):
    """"""
    logname = path
    ...
    handler = ParallelTimedRotatingFileHandler(logname, when="MIDNIGHT", backupCount=2)

    c_format = logging.Formatter('%(asctime)s.%(msecs)03d - %(levelname)s - %(message)s', datefmt="%H:%M:%S")
